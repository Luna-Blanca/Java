...
public class Student_ScoreDB_myself {
    public static void main(String[] args) {
        ...
        while (loop) {
            ...
            if (choice.equalsIgnoreCase("등록") || choice.equals("1")) {  /** 1 , 등록일 경우 **/
                if (count < sj.studentCount()) { // 추가한 학생의 수가 전체 길이보다 작을 경우
                    for (int x = count; x < sj.studentCount(); x++) {
                        for (int i = count; i < sj.studentCount(); i++) { // 학생의 점수 받기
