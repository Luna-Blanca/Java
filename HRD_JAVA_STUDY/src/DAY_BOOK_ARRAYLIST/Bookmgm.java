...
public class Bookmgm {
    ...
    public void init() {
        ...
        if (number != 0) {
            booklist = new ArrayList<BookVO>(number);
            showMenu();
        } else {
    ...
    public void insert() {
        if (booklist.size() != 0) {
            ...
            System.out.println("------------>> 도서갯수 : " + booklist.size() + "권");
            select();
     ...
     */
    public BookVO searchIndex() {
        BookVO book = new BookVO();
        System.out.print("도서번호>>");
        String isbn = scan.next();

        for (BookVO vo : booklist) {
            if (isbn.equals(vo.getIsbn())) {
                book = vo;
            }
        }
        return book;
